...
Claude usage prefers the CLI's final per-model totals. If a run ends without
usable final usage, Multica can recover only main-loop input and cache tokens:
split assistant events with the same response ID count once. Output tokens stay
zero when no final count is available; that does not establish that the model
